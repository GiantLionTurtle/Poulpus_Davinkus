...
impl LinearActuator {
  ...
  pub fn length(&self, clamped: bool) -> f64 {
    if clamped {
      return length(&(self.at(1.0) - self.at(0.0)));
    } else {
      return length(&(self.at_virtual(1.0) - self.at_virtual(0.0)));
    }
  }
...
pub struct DeltaModel {
  pub actuators: [LinearActuator; 3],
...
impl DeltaModel {
  ...
  pub fn new_convergent(height: f64, side: f64, arm_length: f64, eot_offset: DVec3, acuator_exclusion_start: f64, actuator_exclusion_end: f64) -> DeltaModel {
  ...
  pub fn forward_kinematics(&self, motors_positions: [f64;3]) -> Option<DVec3> {
  ...
  pub fn inverse_kinematics(&self, position: DVec3) -> Option<[f64;3]> {
  ...
  pub fn approximate_work_volume(&self, n_points: i32) -> Vec<DVec3> {
    let mut out: Vec<DVec3> = Vec::new();
    for _ in 0..n_points {
      let motors_positions:[f64;3] = self.gen_rand_motors_positions();
      out.push(self.forward_kinematics(motors_positions).unwrap());
    }
    return out;
  }
  fn gen_rand_motors_positions(&self) -> [f64;3] {
    let mut rng = thread_rng();
    return [self.actuators[0].clamp_parametric(rng.gen()), 
            self.actuators[1].clamp_parametric(rng.gen()), 
            self.actuators[2].clamp_parametric(rng.gen())];
  }
}
...
mod linear_actuator_tests {
  ...
  #[test]
  fn at() {
    let lin_act = LinearActuator::new(DVec3::new(0.0, 0.0, 0.0), DVec3::new(100.0, 0.0, 0.0), 0.0, 0.0, 10.0, 10.0);
  ...
  #[test]
  fn direction() {
    let lin_act = LinearActuator::new(DVec3::new(0.0, 0.0, 0.0), DVec3::new(20.0, 20.0, 20.0), 0.0, 0.0, 10.0, 10.0);
    approx::assert_abs_diff_eq!(lin_act.direction().normalize(), DVec3::new(1.0, 1.0, 1.0).normalize()); 
  }
  #[test]
  fn closest_point() {
    let lin_act = LinearActuator::new(DVec3::new(-100.0, 0.0, 0.0), DVec3::new(100.0, 0.0, 0.0), 0.0, 0.0, 10.0, 10.0);
...
mod model_tests {
  ...
  fn forward_kinematics() {
    ...
    for _ in 0..1000 {
      let motors_positions:[f64;3] = model.gen_rand_motors_positions();
